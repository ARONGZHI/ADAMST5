...
##############不需要运行
# 在获取结局id
outcomeid <- c("ieu-a-1120","ieu-b-4960", "ieu-b-94","ieu-b-96","ieu-a-1129","ieu-a-966","ieu-a-822","ieu-b-85",'ieu-b-89',"ukb-b-14956",
              "finn-b-C3_COLORECTAL_EXALLC","finn-b-C3_COLON_EXALLC","finn-b-C3_RECTUM_EXALLC","finn-b-C3_BLADDER_EXALLC",
             'finn-b-C3_MELANOMA_SKIN',"finn-b-CD2_HODGKIN_LYMPHOMA_EXALLC","finn-b-CD2_NONHODGKIN_NAS_EXALLC","finn-b-C3_STOMACH_EXALLC",
             "finn-b-C3_OESOPHAGUS_EXALLC","finn-b-C3_KIDNEY_NOTRENALPELVIS_EXALLC")
...
repeat{
  try({
    outcome <- extract_outcome_data(snps = c(exposure$SNP,exposure_ieu$SNP),proxies = T,rsq = 0.8,
                                    outcomes = c("ieu-a-1120", "ieu-b-94","ieu-b-96","ieu-a-1129","ieu-a-966",
                                                 "ieu-a-822","ieu-b-85",'ieu-b-89',"ukb-b-14956"))
  })
...
##colorectal cancer
fun <- paste0("wget -c -nv -P data ", "https://storage.googleapis.com/finngen-public-data-r9/summary_stats/finngen_R9_C3_COLORECTAL_EXALLC.gz")
system(fun)
...
fun5 <- paste0("wget -c -nv -P data ", "https://storage.googleapis.com/finngen-public-data-r9/summary_stats/finngen_R9_C3_KIDNEY_NOTRENALPELVIS_EXALLC.gz")
system(fun5)

# 食管
fun6 <- paste0("wget -c -nv -P data ", "https://storage.googleapis.com/finngen-public-data-r9/summary_stats/finngen_R9_C3_OESOPHAGUS_EXALLC.gz")
system(fun6)

# 胃
fun7 <- paste0("wget -c -nv -P data ", "https://storage.googleapis.com/finngen-public-data-r9/summary_stats/finngen_R9_C3_STOMACH_EXALLC.gz")
system(fun7)

# 非霍奇金
fun8 <- paste0("wget -c -nv -P data ", "https://storage.googleapis.com/finngen-public-data-r9/summary_stats/finngen_R9_C3_NONHODGKIN_EXALLC.gz")
system(fun8)
...
## 读入芬兰数据形成结局文件
files <- list.files("data",pattern = ".gz")[-c(1,14)]
...
outcome <- outcome_finn %>%
  dplyr::mutate(originalname.outcome = outcome,
                ...
                proxy_a1.outcome = NA,
                proxy_a2.outcome = NA) %>%
  select(SNP,chr.outcome,pos.outcome,beta.outcome,se.outcome,
  ...
  rename(chr = "chr.outcome",pos = "pos.outcome")  %>%
  rbind(exposure_ieu)

saveRDS(outcome,file = "data/canceroutcome.rds")

######
fun11 <- paste0("wget -c -nv -P data ","https://gwas.mrcieu.ac.uk/files/ieu-a-1129/ieu-a-1129.vcf.gz") 
system(fun11)

fun12 <- paste0("wget -c -nv -P data ","https://gwas.mrcieu.ac.uk/files/ieu-a-1120/ieu-a-1120.vcf.gz") 
system(fun12)

fun13 <- paste0("wget -c -nv -P data ","https://gwas.mrcieu.ac.uk/files/ieu-b-85/ieu-b-85.vcf.gz") 
system(fun13)

fun15 <- paste0("wget -c -nv -P data ","https://gwas.mrcieu.ac.uk/files/ieu-b-96/ieu-b-96.vcf.gz") 
system(fun15)

fun15 <- paste0("wget -c -nv -P data ","https://gwas.mrcieu.ac.uk/files/prot-c-3168_8_2/prot-c-3168_8_2.vcf.gz") 
system(fun15)
